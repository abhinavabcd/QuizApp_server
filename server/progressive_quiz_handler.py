...
import json
from server import configuration, gcm_utils
...
def generateProgressiveQuiz(quizId , uids):
    ...
    questions = Questions.getRandomQuestions(quiz)
...
class ProgressiveQuizHandler(tornado.websocket.WebSocketHandler):
    ...
    def open(self, user = None):
        ...
        self.quiz = quiz = Quiz.getQuizDetails(quizId)
        ...
        if(isChallenge!=None):#send notification to other user
            gcm_utils.addUidToQueue(self.isChallenge, {"fromUser":self.uid,
        ...
        if(len(quizConnections)>=int(quiz.nPeople)):# we have enough people
            ...
            self.broadcastToAll({"messageType":LOAD_QUESTIONS,
                                               ...
                                               "payload1":"["+",".join(map(lambda uid: Users.getUserByUid(uid).toJson() , uids))+"]",
...
#                                            "payload":question.to_json(),
            ...
            self.write_message(json.dumps({"messageType":OK_ACTIVATING_BOT, "payload1": BotUids.getBotUser().toJson(), 
                                           "payload2":"["+",".join(map(lambda x:x.to_json() ,Questions.getRandomQuestions(self.quiz)))+"]"}))
            ...
            if(len(currentRequests)>=self.quiz.nPeople):#every one agreed to rematch
                questions = Questions.getRandomQuestions(self.quiz)
                resetQuizState(self.runningQuiz, questions)
                self.broadcastToAll(json.dumps({"messageType":OK_START_REMATCH,"payload": BotUids.getBotUser().toJson(), 
            ...
            self.write_message(json.dumps({"messageType":OK_CHALLENGE_WITHOUT_OPPONENT ,"payload1": Users.getUserByUid(self.isChallenge).toJson(), 
                                           "payload2":"["+",".join(map(lambda x:x.to_json() ,Questions.getRandomQuestions(self.quiz)))+"]",
